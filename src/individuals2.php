...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
        <h2>Home Teacher Assignments</h2>
<?php
...
if (!empty($assigned)) {
    ...
    echo '<tr>';
    echo '<th>Name</th>';
    echo '<th colspan="4">Teacher1</th>';
    echo '<th colspan="4">Teacher2</th>';
    echo '</tr>';
    echo '</thead>';
    echo '<tbody>';
    foreach ($sort as $id) {
        if (in_array($id, $assigned) && !in_array($id, $ntList)) {
            ...
            if ($mem->Gender == 'FEMALE') {
                ...
                echo '<tr>';
                echo '<td scope="row">'.$mem->Name.'</td>';
                foreach ($companionships[$companionshipID] as $teacher) {
                    $tMem = $mngr->getMemberByID($teacher);
                    echo '<td scope="row">'.$tMem->Name.'</td>';
                    echo '<td scope="row">'.$tMem->Phone.'</td>';
                    if (!empty($tMem->Photo1) || !empty($tMem->Photo2)) {
                        if (!empty($tMem->Photo1) && !empty($tMem->Photo2)) {
                            echo '<td><a href="#" class="pop"><img class="listImage" src="'.$tMem->Photo1.'" /></a></td>';
                            echo '<td><a href="#" class="pop"><img class="listImage" src="'.$tMem->Photo2.'" /></a></td>';
                        } elseif (!empty($tMem->Photo1)) {
                            echo '<td><a href="#" class="pop"><img class="listImage" src="'.$tMem->Photo1.'" /></a></td>';
                            echo '<td>&nbsp;</td>';
                        } else {
                            echo '<td><a href="#" class="pop"><img class="listImage" src="'.$tMem->Photo2.'" /></a></td>';
