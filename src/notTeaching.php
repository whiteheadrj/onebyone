...
<html lang="en">
  ...
  <body>
      ...
      <div class="container">
        ...
        <div id="navbar" class="collapse navbar-collapse">
          <ul class="nav navbar-nav">
            ...
            <li class="active"><a href="./notTeaching.php">Not Teaching</a></li>
    ...
    <div class="container">
        <h2>Mark Individuals Not Teaching</h2>
<?php

if (!empty($sort)) {
    echo '<form method="post" action="notTeachingSave.php">';
    ...
    foreach ($sort as $id) {
        ...
        $checked = '';
        if (in_array($id, $ntList)) {
            $checked = ' checked="checked"';
        }

        $reason = '';
        if (array_key_exists($id, $ntReasons)) {
            $reason = $ntReasons[$id];
        }
        echo '<td><input'.$checked.' class="form-check-input" type="checkbox" name="nt[]" value="'.$id.'" /></td>';
        echo '<td><input type="input" name="ntReasons['.$id.']" size="50" value="'.$reason.'" /></td>';
        echo '</tr>';
    }
    echo '</tbody>';
    echo '</table>';
