...
echo '<h2>With Compaion</h2>';
if (!empty($males) && $cmngr->countOfCompanionships()) {
    echo '<div class="table-responsive">';
    echo '<table class="table">';
    echo '<thead class="thead-default">';
    echo '<tr>';
    ...
    foreach ($sort as $id) {
        ...
        if (in_array($id, $males) && count($companions) > 0) {
            $mem = $mngr->getMemberByID($id);
            echo '<tr>';
            if (!empty($mem->Photo1) || !empty($mem->Photo2)) {
                if (!empty($mem->Photo1) && !empty($mem->Photo2)) {
            ...
            echo '<td scope="row">'.$mngr->getMemberByID($id)->Name.'</td>';
            echo '<td scope="row">'.$mngr->getMemberNamesByIDArray($companions).'</td>';
            echo '<td><a class="btn btn-default" href="companionshipSave.php?action=rmFromCompanionship&amp;id='.$id.'">Remove from Companionship</a></td>';
            echo '</tr>';
        }
    }
    echo '</tbody>';
